...
#train, test = datasets.IMDB.splits(text_field=SRC,label_field=TGT)
...
BATCH_SIZE = 1000
train_iter = MyIterator(trnset, batch_size=BATCH_SIZE, device=0,
                        repeat=False, sort_key=lambda x: (len(x.src), len(x.trg)),
                        batch_size_fn=batch_size_fn, train=True)
valid_iter = MyIterator(testset, batch_size=BATCH_SIZE, device=0,
                        repeat=False, sort_key=lambda x: (len(x.src), len(x.trg)),
...
torch.save(model.state_dict(), './onnx.pth')

trained_model = make_model(V, V, N=2)
trained_model.load_state_dict(torch.load('./onnx.pth'))
dummy_input = Variable(torch.from_numpy(np.random.randint(1, V, size=(128, 10))))
torch.onnx.export(trained_model,
(dummy_input,dummy_input,(src != SRC.vocab.stoi["<blank>"]).unsqueeze(-2),(src != SRC.vocab.stoi["<blank>"]).unsqueeze(-2)) 
,"./onnx1.onnx", opset_version=10,verbose=True,
                       operator_export_type=torch.onnx.OperatorExportTypes.ONNX_ATEN_FALLBACK)

model = onnx.load('./onnx1.onnx')
tf_rep = prepare(model) 


output = tf_rep.run([np.random.randint(1, V, size=(128, 10)),np.random.randint(1, V, size=(128, 10)),(src != SRC.vocab.stoi["<blank>"]).unsqueeze(-2),(src != SRC.vocab.stoi["<blank>"]).unsqueeze(-2)])
