...
class SimpleLossCompute:
    ...
    def __call__(self, x, y, norm):
        ...
        loss = self.criterion(torch.sum(x,(0)), 
                              torch.sum(y,(0))) / norm  #### FIX HERE 0 OU 1
...
for i in range(0,memory.shape[0]):
    ...
    result.append(torch.sum(torch.sum(prob,(0)),(0)).detach().numpy())
    #print(torch.sum(prob,(1)).detach().numpy())
...
plt.plot(train[9:-1].reshape(1,-1)[0])
