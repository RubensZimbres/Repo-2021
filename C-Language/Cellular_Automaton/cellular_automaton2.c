...
{   int inicial[6]={1,0,0,1,0,1};
    ...
    split_array[0][0]=inicial[n];
    split_array[0][1]=inicial[0];
    split_array[0][2]=inicial[1];
    split_array[n][0]=inicial[(int)(n-1)];
    split_array[n][1]=inicial[n];
    split_array[n][2]=inicial[0];
    for (k=1;k<n;k++){
        split_array[k][0]=inicial[(int)(k-1)];
        split_array[k][1]=inicial[k];
        split_array[k][2]=inicial[(int)(k+1)];
    }
    int i=0;
    int posicao[8];
    int states=2;
