...
class Embeddings1(nn.Module):
    ...
    def forward(self, x):
        return torch.cat(32*[x]).reshape(100,8,self.d_model)  * math.sqrt(self.d_model)
...
class Embeddings2(nn.Module):
    ...
    def forward(self, x):
        return torch.cat(32*[x]).reshape(100,7,self.d_model)  * math.sqrt(self.d_model)
...
def make_model(src_vocab, tgt_vocab, N=6, 
               d_model=32, d_ff=256, h=8, dropout=0.1):
